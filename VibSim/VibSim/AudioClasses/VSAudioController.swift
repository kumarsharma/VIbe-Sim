...
class VSAudioController: NSObject, AURenderCallbackDelegate {
        ...
        ioData: UnsafeMutablePointer<AudioBufferList>) -> OSStatus
    {
        ...
        if !audioChainIsBeingReconstructed {
            ...
            if _bufferManager.displayMode == .oscilloscopeWaveform {
                ...
                _bufferManager.copyAudioDataToDrawBuffer(ioPtr[0].mData?.assumingMemoryBound(to: Float32.self), inNumFrames: Int(inNumberFrames), dataSize: ioPtr[0].mDataByteSize)
